...
function AddCar() {
  ...
  const fetchRentedCars = async () => {
    try {
      const res = await api.get('/api/cars/my', {
  ...
  const handleDeleteCar = async (carId) => {
    ...
    try {
      await api.delete(`/api/cars/my/${carId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setRentedCars((prev) => prev.filter((car) => car._id !== carId));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!CLOUDINARY_CLOUD_NAME || !CLOUDINARY_UPLOAD_PRESET) {
        throw new Error('Cloudinary frontend env vars are missing');
      }

      const cloudinaryFormData = new FormData();
      cloudinaryFormData.append('file', form.image);
                    ...
                    <div key={car._id} className="bg-gray-700 rounded-xl overflow-hidden shadow hover:shadow-lg transition duration-300">
                      ...
                      <div className="p-5">
                        ...
                        <div className="flex items-center text-gray-400 text-sm mb-3">
                          <span className="capitalize">{car.type || 'vehicle'}</span>
                          <span className="mx-2">•</span>
                          <span>
                            Added on {car.createdAt && !Number.isNaN(new Date(car.createdAt).getTime())
                        ...
                        <div className="flex justify-between items-center pt-3 border-t border-gray-600">
                          ...
                          <button
                            ...
                            className="text-sm bg-gray-600 hover:bg-red-600 disabled:opacity-70 px-3 py-1 rounded transition duration-300"
                          >
                            {deletingCarId === car._id ? 'Deleting...' : 'Manage'}
                          </button>
                        </div>
                      </div>
                    </div>
      ...
      {selectedCar && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-gray-800 rounded-xl w-full max-w-lg overflow-hidden border border-gray-700">
            ...
            <div className="p-6">
              <h3 className="text-2xl font-bold mb-2">{selectedCar.name}</h3>
              <p className="text-amber-400 font-semibold mb-3">₹{selectedCar.price}/day</p>
              <p className="text-sm text-gray-400 capitalize mb-3">Type: {selectedCar.type || 'vehicle'}</p>
              <div className="flex flex-wrap gap-2 mb-6">
                {(selectedCar.specs || []).length > 0 ? (
                  selectedCar.specs.map((spec, idx) => (
                    <span key={idx} className="text-xs bg-gray-700 text-amber-300 px-2 py-1 rounded">
                      {spec}
                    </span>
                  ))
                ) : (
              ...
              <div className="flex justify-end">
                <button
                  ...
                  className="bg-amber-500 hover:bg-amber-400 text-gray-900 font-semibold px-4 py-2 rounded"
                >
                  Close
                </button>
              </div>
